...
<div class="container-fluid px-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <div class="flex items-center space-x-4">
            @if($unreadCount > 0)
            <span class="bg-red-500 text-white text-xs font-semibold px-2.5 py-0.5 rounded">
                {{ $unreadCount }} Unread
            </span>
            @endif
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <div class="bg-white rounded-lg shadow p-4">
            ...
            <div class="text-2xl font-bold text-gray-800">{{ $messages->total() }}</div>
        </div>
        <div class="bg-white rounded-lg shadow p-4">
            <div class="text-sm font-medium text-gray-500">Unread</div>
            <div class="text-2xl font-bold text-red-600">{{ $unreadCount }}</div>
        </div>
        <div class="bg-white rounded-lg shadow p-4">
            <div class="text-sm font-medium text-gray-500">Replied</div>
            <div class="text-2xl font-bold text-green-600">{{ \App\Models\ContactMessage::replied()->count() }}</div>
        </div>
        <div class="bg-white rounded-lg shadow p-4">
            <div class="text-sm font-medium text-gray-500">Read</div>
            <div class="text-2xl font-bold text-blue-600">{{ \App\Models\ContactMessage::read()->count() }}</div>
        </div>
    </div>
