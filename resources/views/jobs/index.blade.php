                        ...
                        @if (!in_array($key, ['page', '_token']) && !empty($value))
                            @if (is_array($value))
                                @foreach ($value as $item)
                                    <span
                                        class="bg-indigo-100 text-indigo-800 text-xs px-2 py-1 rounded-full inline-flex items-center">
                                        @if ($key == 'categories')
                                            {{ $categories->where('id', $item)->first()->name ?? $item }}
                                        @else
                                            {{ $item }}
                                        @endif
                                        <a href="{{ removeFilter($key, $item) }}"
        ...
        <div class="flex flex-col lg:flex-row gap-6">
            ...
            <div class="lg:w-64 flex-shrink-0">
                <div class="bg-white rounded-lg border border-gray-200 p-5 sticky top-6">
                    <h3 class="text-lg text-center font-semibold text-gray-900 mb-4">Filter Jobs: <span
                            class="text-blue-700">{{ $jobs->total() }}</span></h3>
    ...
    @endif
                ...
                @if ($jobs && $jobs->count() > 0)
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        @foreach ($jobs as $job)
                            <div
                                class="bg-white rounded-lg border border-gray-200 hover:border-indigo-300 hover:shadow transition-all h-full flex flex-col">
                                <div class="p-4 flex-1 flex flex-col">
                                    ...
                                    <div class="flex items-start gap-3 mb-3">
                                        ...
                                        @endif
                                        <div class="flex-1 min-w-0">
                                            <h3 class="font-semibold text-gray-900 text-bold text-sm mb-1 truncate"
                                                title="{{ $job->title }}">
                                    ...
                                    <div class="space-y-2 mb-3 flex-1">
                                        <div class="flex items-center text-xs text-gray-600">
                                            <svg class="w-3.5 h-3.5 mr-1.5 text-gray-400 flex-shrink-0" fill="none"
